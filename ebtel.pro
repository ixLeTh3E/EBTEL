pro ebtel, ttime, heat, length, t, n, p, v, ta, na, pa, c11, dem_tr, dem_cor,  $
     ...
     ;   dynamic   = set to use dynamical r1 and r2 (NOT recommended, specially when T > 10 MK). Now redundant.
     ...
     ;      IDL> ebtel, ttime, heat, t, n, p, v, dem_tr, dem_cor, logtdem
     ;   To exclude the transition region DEM (faster):
     ;      IDL> ebtel, ttime, heat, t, n, p, v
     ;   To include a nonthermal electron energy flux:
     ;      IDL> ebtel, ttime, heat, t, n, p, v, flux_nt=flux_nt
     ;   To compute rad_ratio:
     ;      IDL> ebtel, ttime, heat, t, n, p, v, dem_tr, dem_cor, logtdem, f_ratio, rad_ratio
     ...
     if not keyword_set(flux_nt) then f_nt = ttime*0. else f_nt = -flux_nt
     
     if not keyword_set(energy_nt) then energy_nt = 50.   ; 50 keV
     j_nt = 6.241e8*f_nt/energy_nt
...
;        nn = 1e9/r2
  ...
  ;   ________________
   ...
   ;    if not keyword_set(flow_correction) then n_fact(i)=1.
       
       pv = 0.4*(f_eq - f - f_nt(i))
       ;pv = 0.4*(f_eq - f - f_nt(i))
  ...
  ;     dp = 2./3.*(q(i) +(f+ 1./r3*f_eq)/length    $
  ;       +1.5*k_b*t(i)/energy_nt*6.241e8*f_nt(i)/length)*dt +5./3.*pv/length*dt
   
  ;     dp = 2./3.*(q(i) +(f+ 1./r3*f_eq)/length    $
  ;       +1.5*k_b*t(i)/energy_nt*6.241e8*f_nt(i)/length)*dt +5./3.*n_fact(i)*pv/length*dt
       dp = 2./3.*(q(i) + (1. + 1./r3)*f_eq/length    $
         - (1. - 1.5*k_b*t(i)/energy_nt*6.241e8)*f_nt(i)/length)*dt     
     ...
     ;  if keyword_set(flow_correction) then begin
     ;   cs=cs_fact*sqrt(t(i)*r12 )
     ;   v(i+1)=(v(i+1)*cs)/sqrt(v(i+1)*v(i+1)+cs*cs) 
     ;  endif
       
       
       h_tot = h_tot+heat(i)*dt
       
       if ((n[i+1] le 0) or (p[i+1] le 0) ) then begin
           print, 'Catastrophic condensation! Calculation terminated.'
           goto, jump99
       endif
